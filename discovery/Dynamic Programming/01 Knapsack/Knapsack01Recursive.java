public class Knapsack01Recursive {
    private static int knapsack(int[] itemProfits, int[] itemWeights, int knapsackCapacity, int itemsIncluded) {
        if(itemsIncluded < 1 || itemWeights[itemsIncluded-1] > knapsackCapacity)
            return 0;
        
        int resultWhenIncludingCurrentItem = itemProfits[itemsIncluded-1] + knapsack(itemProfits, itemWeights, knapsackCapacity-itemWeights[itemsIncluded-1], itemsIncluded-1);
